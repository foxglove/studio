...
import { generateStudioLib, generateTypesInterface } from "./generateStudioLib";
...
describe("generateStudioLib", () => {
  describe("generateDatatypesInterface", () => {
    it("should generate basic Datatypes interface", async () => {
      ...
      expect(prettified).toEqual(`declare interface Types {
  "std_msgs.ColorRGBA": {
    r: number;
    g: number;
    b: number;
...
`);
  ...
  it("full", async () => {
    ...
    expect(prettified).toEqual(`type Time = {
  sec: number;
  nsec: number;
};
...
declare interface TypesByTopic {
  "/my_topic": MessageEvent<Types["std_msgs/ColorRGBA"]>;
}
