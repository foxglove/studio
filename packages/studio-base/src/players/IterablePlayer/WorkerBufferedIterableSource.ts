...
export class WorkerBufferedIterableSource<MessageType = unknown>
  implements IIterableSource<MessageType>
{
  readonly #args: ConstructorArgs;

  #sourceWorkerRemote?: Comlink.Remote<WorkerBufferedIterableSourceWorker<MessageType>>;
  #disposeRemote?: () => void;
  ...
  public async initialize(): Promise<Initalization> {
    ...
    const { remote: initializeWorker, dispose } =
      ComlinkWrap<
        ...
        ) => Comlink.Remote<WorkerBufferedIterableSourceWorker<MessageType>>
      >(worker);
  ...
  public async stopProducer(): Promise<void> {
    await this.#sourceWorkerRemote!.stopProducer();
  }

  public async getCacheSize(): Promise<number> {
    return await this.#sourceWorkerRemote!.getCacheSize();
  }

  public async loadedRanges(): Promise<Range[]> {
    return await this.#sourceWorkerRemote!.loadedRanges();
  }
  ...
  public async onLoadedRangesChange(
    ...
    await this.#sourceWorkerRemote?.onLoadedRangesChange(
