...
describe("BufferedIterableSource", () => {
  it("should construct and initialize", async () => {
    ...
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0 }]);
  });
  ...
  it("should produce messages that the source produces", async () => {
    ...
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 1 }]);
  });
  ...
  it("should indicate loading ranges when partially buffered", async () => {
    ...
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.4999999999 }]);
  ...
  it("should yield correct messages when starting a new iterator before the the cached items", async () => {
...

    {
      ...
      await doneYield.wait();
      expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0.5, end: 1 }]);
  ...
  it("should wait to buffer more messages until reading moves forward", async () => {
    ...
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.1999999999 }]);

    // Reading the second message buffers more data
    signal = waiter(1);
    await messageIterator.next();
    await signal.wait();
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.2999999999 }]);
  ...
  it("should buffer minimum duration ahead before messages can be read", async () => {
    ...
    await messageIterator.next();
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.3999999999 }]);
    // Let the buffer read to the end
    await signal.wait();
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.5999999999 }]);
  ...
  it("should support stamp iterator results and wait to buffer more messages until reading moves forward", async () => {
    ...
    expect(bufferedSource.getLoadedRanges().ranges).toEqual([{ start: 0, end: 0.0999999999 }]);
