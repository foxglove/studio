...
function RosHostname(): React.ReactElement {
  ...
  return (
    <TextField
      ...
      value={rosHostname ?? ""}
      onChange={async (_event, newValue) => setRosHostname(nonEmptyOrUndefined(newValue))}
    />
  );
}
        ...
        <Stack.Item>
          ...
          <Stack tokens={{ childrenGap: theme.spacing.s1 }}>
            ...
            <Checkbox
              ...
              onChange={async (_event, checked) => setTelemetryEnabled(checked)}
              label={`Send anonymized usage data to help us improve Foxglove Studio`}
            />
            <Checkbox
              checked={crashReportingEnabled ?? true}
              onChange={async (_event, checked) => setCrashReportingEnabled(checked)}
