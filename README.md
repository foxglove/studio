...
[Bind-mount](https://docs.docker.com/storage/bind-mounts/) a layout JSON file at `/etc/foxglove/default-layout.json` to set the default layout used when loading Studio from the Docker image.

```sh
docker run --rm -p "8080:8080" -v /path/to/custom_layout.json:/etc/foxglove/default-layout.json ghcr.io/foxglove/studio:latest
```
