...
export default function debouncePromise(fn: (...args: unknown[]) => Promise<void>): DebouncedFn {
  ...
  let callPending: unknown[] | null | undefined;

  const debouncedFn: DebouncedFn = (...args: unknown[]) => {
    if (calling) {
      callPending = args;
    } else {
      start(args);
